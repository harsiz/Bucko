...
your chaotic, funny, weirdly self-aware desktop companion.
(NAME NOT FULLY DECIDED YET LOOL)
...
it's honestly kind of unhinged once you've been using it for a while lmaooo.
...
- **Discord RPC (Discord Activity Status)** >> shows what session number you're on. fails silently if Discord isn't open.
- **typewriter text** >> character-by-character rendering with configurable speed and inline waits.
- **local only** >> everything is saved in `saves/player.dat`. SHA-256 hashed for integrity. nothing leaves your big boy machine 🤤🤤🤤.
...
nothing is ever sent anywhere. not for api requests, not for bug fixes, no analytics, no cloud sync. if you delete that file, Bucko forgets you entirely. the file is SHA-256 hashed so if you manually edit it, Bucko notices and logs a warning >> but it still loads.
